...
# This function using ggplot2 package to better visualize ACF & PACF plots

ggplot.acf <- function(data, lag.max = 24, ci = 0.95, large.sample.size = TRUE, horizontal = TRUE,...) {
  ...
  if(large.sample.size == TRUE) {
    ...
    geom_area(aes(x = lag, y = -qnorm((1+ci)/2)*acfstd), fill = "#B9CFE7") +
    geom_col(fill = "#00ab4e", width = 0.7) +
